...
class MyceliumCompanion:
    ...
    def __init__(
        ...
        self.vault: Any = None
        self._system: Any = None  # MyceliumSystem (optional heavy dep)
    ...
    def start(self) -> None:
        ...
        self._started = True
        _logger.info("MyceliumCompanion %r started", self.name)
    ...
    def _dispatch(self, text: str, intent: str, entities: Dict[str, Any]) -> str:
        ...
        """
        # Document processing (always deterministic — file I/O)
        paths = entities.get("paths", [])
        if paths and intent in ("train", "process", "read", "load", "analyze", "analyse", "unknown"):
    ...
    def _handle_predict(self, text: str, entities: Dict[str, Any]) -> str:
        ...
        if self._system is not None:
            try:
                import numpy as np
                if numbers:
                    result = self._system.step(np.array(numbers))
                    return self.formatter.format_action_result(result)
            except Exception as e:
                _logger.warning("MyceliumCompanion: predict failed: %s", e)
        # Generic response
        self.soul.increment_stat("total_predictions")
        self.soul.record_event("prediction", description="User requested prediction")
        conf = min(0.5 + len(numbers) * 0.05, 0.95)
        return self.formatter.format_prediction(
            prediction="N/A (no model loaded)",
            confidence=conf,
            feature_names=[f"x{i}" for i in range(len(numbers))] or None,
        )
    ...
    def _handle_train(self, text: str, entities: Dict[str, Any]) -> str:
        ...
        return (
            f"To train {self.name}, provide a data file path. "
    ...
    def _handle_llm(self, text: str) -> str:
        ...
        # Build conversation history from ConversationManager turns
        history: List[LLMMessage] = []
        if self.conversation is not None:
            for turn in list(self.conversation.turns)[-20:]:
                role = "user" if turn.speaker == "user" else "assistant"
                history.append(LLMMessage(role=role, content=turn.text))

        # Build system prompt from soul + profile
        ...
        result = self.llm.chat(
            user_message=text,
            history=history[:-1],  # exclude last user turn (already passed as user_message)
            system=system,
        )
        ...
        return (
            f"I couldn't process that request right now. "
            f"Try: 'predict', 'train on <file>', 'show report', or '/help'."
        )

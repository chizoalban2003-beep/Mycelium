...
def run_physics_prediction(
    ...
    *,
    ...
    # When provided, overrides the random train/test split so the caller can pass
    # pre-determined train/test row masks (e.g. from an external cross-validation split).
    # Must be a boolean array of length df.shape[0]; True = train row.
    ...
    # When an explicit_train_mask is supplied the caller owns the row layout, so skip row-dropping
    # operations (deduplication / missing-target removal) that would change the row count.
    cleaning_diag: dict[str, Any] | None = None
    if bool(cleaning_enabled):
        _drop_dups = bool(cleaning_drop_duplicates) and explicit_train_mask is None
        _drop_missing = bool(cleaning_drop_missing_target) and explicit_train_mask is None
        df, cleaning_diag = _clean_dataframe_for_prediction(
            df,
            target_col=target_col,
            train_mask=None,
            drop_duplicates=_drop_dups,
            drop_missing_target=_drop_missing,
    ...
    if explicit_train_mask is not None:
        _etm = np.asarray(explicit_train_mask, dtype=bool)
        if _etm.shape[0] != int(df.shape[0]):
            raise PredictorError(
                f"explicit_train_mask length {_etm.shape[0]} != df rows {df.shape[0]}"
            )
        train_mask = _etm
        test_mask = ~_etm
    else:
    ...
    if bool(enable_isotopes):
        ...
        if isotope_cols:
            if explicit_train_mask is not None:
                # Isotope bundling didn't change row count; mask stays valid.
                pass
            else:
                train_mask, test_mask = _train_test_split_mask(int(df.shape[0]), train_fraction, random_seed)
    ...
    if target_kind in ("numeric", "datetime"):
        ...
        _numeric_result = PredictionResult(
            target=target_col,
            target_kind=target_kind,
            plane=plane,
